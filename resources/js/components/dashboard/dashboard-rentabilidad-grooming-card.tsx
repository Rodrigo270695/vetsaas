...
export function DashboardRentabilidadGroomingCard({
    initial,
    moneda,
    locale,
    endpoint = '/reportes/financiero/rentabilidad-grooming',
}: Props) {
    ...
    const fetchData = useCallback((nextPeriodo: RentabilidadPeriodo, nextFiltros: RentabilidadComprobanteFiltros) => {
        ...
        fetch(buildRentabilidadUrl(endpoint, nextPeriodo, nextFiltros), {
            ...
            .then((payload: RentabilidadGroomingResumen) => {
    ...
    return (
        <Card className="min-w-0 gap-0 overflow-hidden border-border/80 py-0 shadow-sm transition-shadow hover:shadow-md">
            <CardHeader className="border-b border-sky-200/60 bg-linear-to-br from-sky-50/95 via-sky-50/50 to-sky-50/10 pb-4 pt-5 dark:border-sky-800/40 dark:from-sky-950/50 dark:via-sky-950/25 dark:to-transparent">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="flex items-start gap-3">
                        <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-sky-100 text-sky-700 ring-1 ring-sky-200/50 dark:bg-sky-900/40 dark:text-sky-200">
                            <Scissors className="size-4" aria-hidden />
                        </div>
                        <div className="min-w-0">
                            <h3 className="text-base font-semibold text-foreground">
                                {t('rentabilidad_grooming.title')}
                            </h3>
                            <p className="mt-0.5 text-sm text-muted-foreground">
                                {t('rentabilidad_grooming.subtitle')}
                            </p>
                        </div>
                    </div>
                    ...
                    <div
                        role="group"
                        aria-label={t('rentabilidad_grooming.title')}
                    ...
                    >
                        {PERIODOS.map((p) => {
                            ...
                            return (
                                ...
                                >
                                    {t(`rentabilidad_grooming.periodo.${p}`)}
                                </button>
                            );
                        })}
                    </div>
                ...
                <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                    {loading ? (
                        <>
                            ...
                            {t('rentabilidad_grooming.loading')}
                        </>
                    ) : (
            ...
            <CardContent className="min-w-0 space-y-5 bg-muted/20 px-6 pb-6 pt-5">
                {error ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border/80 bg-background/60 px-6 py-10 text-center">
                        <div className="flex size-12 items-center justify-center rounded-full bg-sky-50 text-sky-600 dark:bg-sky-950/50 dark:text-sky-300">
                            <Scissors className="size-6" aria-hidden />
                        </div>
                        <p className="max-w-xs text-sm text-muted-foreground">
                            {t('rentabilidad_grooming.empty')}
                        </p>
                    </div>
                ) : (
                    <div className={cn('space-y-5 transition-opacity', loading && 'opacity-60')}>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
                                <span>{t('rentabilidad_grooming.composicion')}</span>
                                <span className="tabular-nums">
                                    {data.unidades.toLocaleString(locale)} {t('rentabilidad_grooming.unidades')}
                                </span>
                            </div>
                            <div className="flex h-3 w-full overflow-hidden rounded-full bg-muted">
                                <div
                                    ...
                                    title={`${t('rentabilidad_grooming.costo_label')}: ${formatMoney(data.costo, moneda, locale)}`}
                                />
                                <div
                                    className={cn('h-full transition-all', data.ganancia < 0 ? 'bg-rose-500' : 'bg-emerald-500')}
                                    style={{ width: `${gananciaPct}%` }}
                                    title={`${t('rentabilidad_grooming.ganancia_label')}: ${formatMoney(data.ganancia, moneda, locale)}`}
                                />
                            </div>
                            <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                                <span className="inline-flex items-center gap-1.5">
                                    <span className="size-2.5 rounded-sm bg-amber-400/80" />
                                    {t('rentabilidad_grooming.costo_label')} · {costoPct.toFixed(0)}%
                                </span>
                                <span className="inline-flex items-center gap-1.5">
                                    <span className={cn('size-2.5 rounded-sm', data.ganancia < 0 ? 'bg-rose-500' : 'bg-emerald-500')} />
                                    {t('rentabilidad_grooming.ganancia_label')} · {gananciaPct.toFixed(0)}%
                                </span>
                            </div>
                        </div>
                        ...
                        {data.items.length > 0 && (
                            <div className="space-y-2">
                                <p className="text-xs font-medium text-muted-foreground">
                                    {t('rentabilidad_grooming.top_title')}
                                </p>
                                        ...
                                        <thead>
                                            <tr className="border-b border-border/60 bg-muted/40 text-xs text-muted-foreground">
                                                <th className="px-3 py-2 text-left font-medium">
                                                    {t('rentabilidad_grooming.col_servicio')}
                                                </th>
                                                <th className="px-3 py-2 text-right font-medium">
                                                    {t('rentabilidad_grooming.col_uds')}
                                                </th>
                                                <th className="px-3 py-2 text-right font-medium">
                                                    {t('rentabilidad_grooming.col_ganancia')}
                                                </th>
                                                <th className="px-3 py-2 text-right font-medium">
                                                    {t('rentabilidad_grooming.margen')}
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                            ...
                            <div className="flex items-start gap-2.5 rounded-lg border border-amber-200/70 bg-amber-50/60 px-3 py-2.5 text-xs text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
                                ...
                                <div className="space-y-0.5">
                                    <p className="font-medium">
                                        {t('rentabilidad_grooming.sin_insumos_aviso', { count: data.servicios_sin_insumos })}
                                    </p>
                                    <p className="opacity-80">{t('rentabilidad_grooming.sin_insumos_cta')}</p>
                                </div>
                            </div>
                        )}
