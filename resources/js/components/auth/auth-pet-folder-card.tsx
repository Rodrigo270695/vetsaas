...
export function AuthCollabCard({ className }: { className?: string }) {
    return (
        <div
            aria-hidden
            className={cn(
                'pointer-events-none absolute w-64 rounded-2xl border border-border/60 bg-card/85 p-4 text-left shadow-[0_20px_60px_-30px_rgba(0,40,30,0.35)] backdrop-blur-xl dark:bg-card/60',
                className,
            )}
        >
            <div className="relative mx-auto h-28 w-[11.5rem]">
                <div className="absolute inset-x-2 top-2 rounded-xl border border-border/70 bg-zinc-900 p-3 shadow-lg">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="h-1.5 w-16 rounded-full bg-sky-400/80" />
                        <div className="h-1.5 w-12 rounded-full bg-emerald-400/80" />
                        <div className="h-1.5 w-20 rounded-full bg-zinc-600" />
                        <div className="h-1.5 w-8 rounded-full bg-violet-400/60" />
                    </div>
                </div>
                <span className="absolute top-8 right-3 text-lg leading-none text-emerald-400">↗</span>
                <div className="absolute right-0 bottom-1 flex flex-col items-end gap-1">
                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500 px-2 py-0.5 text-[0.65rem] font-semibold text-white shadow-sm">
                        <span className="size-3.5 rounded-full bg-emerald-200" />
                        Agenda
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-full bg-sky-500 px-2 py-0.5 text-[0.65rem] font-semibold text-white shadow-sm">
                        <span className="size-3.5 rounded-full bg-sky-200" />
                        Equipo
                    </span>
                </div>
            </div>
            <p className="mt-3 text-sm font-semibold text-foreground">Trabajo en equipo</p>
            <p className="text-xs leading-relaxed text-muted-foreground">
                Agenda, caja e historia en un mismo espacio de trabajo.
            </p>
        </div>
    );
}
