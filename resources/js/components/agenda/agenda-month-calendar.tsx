...
type Props = {
    ...
    canCreate: boolean;
    canUpdate?: boolean;
    legend: readonly AgendaLegendItem[];
    labels: AgendaCalendarLabels;
    onSelectEvent: (event: AgendaEvent) => void;
    onScheduleDay: (fecha: string, hora?: string) => void;
    onReschedule?: (event: AgendaEvent, fecha: string, hora?: string) => void;
    onPrevMonth: () => void;
    onNextMonth: () => void;
...
export function AgendaMonthCalendar({
    ...
    const eventsByDay = useMemo(() => {
        ...
        for (const event of events) {
            const key = toDateKey(
                new TZDate(event.inicio_at, timeZone),
                timeZone,
            );
    ...
    const activeDay = selectedDay?.startsWith(`${mes}-`)
        ? selectedDay
        : defaultPanelDay;
    const activeDayEvents = eventsByDay.get(activeDay) ?? [];
    ...
    const hourSlots = useMemo(() => {
        ...
        const lastHour =
            eventHours.length > 0
                ? Math.max(configuredEnd, ...eventHours)
                : configuredEnd;
    ...
    }, [events, horaFin, horaInicio, timeZone]);
    ...
    const resolveDragEvent = (e: DragEvent): AgendaEvent | null => {
        const id =
    ...
    const handleDragStart = (e: DragEvent, event: AgendaEvent) => {
        if (!event.canDrag || !canUpdate || !onReschedule) {
            e.preventDefault();

            return;
        }

        e.dataTransfer.setData(DND_MIME, event.id);
    ...
    const handleDropOnDay = (
        ...
        const event = resolveDragEvent(e);

        if (!event?.canDrag || !canUpdate) {
            return;
        }

        setSelectedDay(dateKey);
        onReschedule(event, dateKey);
    };
    ...
    const handleDropOnHour = (e: DragEvent, dateKey: string, hour: number) => {
        ...
        setSelectedDay(dateKey);
        onReschedule(event, dateKey, hora);
    };
        ...
        >
            ...
            <div className="grid lg:grid-cols-[1fr_minmax(17rem,22rem)]">
                <div className="border-b border-border/50 p-3 sm:p-4 lg:border-r lg:border-b-0">
                    ...
                    {canUpdate && onReschedule && labels.dragHint ? (
