...
export default function GroomingCargos({ turno, cargo, cobro, clinic_billing }: Props) {
    const { t, i18n } = useTranslation(['consulta-cargos', 'grooming', 'nav', 'caja']);
    ...
    const puedeEditarCargos = can('consulta-cargos.manage') || can('grooming.update');
    const productosBuscarUrl = `${LIST_URL}/${turno.id}/cargos/productos-buscar`;
    const serviciosBuscarUrl = `${LIST_URL}/${turno.id}/cargos/servicios-buscar`;
    const showUrl = LIST_URL;
    const cargosBaseUrl = `${LIST_URL}/${turno.id}/cargos`;
    ...
    useEffect(() => {
        setLayoutProps({
            breadcrumbs: [
                ...
                { title: t('grooming:title'), href: LIST_URL },
                { title: turno.paciente.nombre, href: showUrl },
    ...
    }, [historiasUrl, showUrl, t, turno.paciente.nombre]);
    ...
    const propietarioLabel = useMemo(() => {
        const pr = turno.paciente.propietario;
        if (!pr) {
            return '—';
        }
    ...
    }, [turno.paciente.propietario]);

    const atendidoLabel = formatAtendidoInAppTimezone(turno.inicio_at, appLocale, appTz);
    ...
    const headerStats = useMemo(
        ...
        [turno.paciente.nombre, esBorrador, propietarioLabel, t],
    );

    const consultaStub = useMemo(
        () => ({
            id: turno.id,
            veterinario: turno.responsable
                ? { id: turno.responsable.id, name: turno.responsable.name }
                : null,
        }),
        [turno.id, turno.responsable],
    );
