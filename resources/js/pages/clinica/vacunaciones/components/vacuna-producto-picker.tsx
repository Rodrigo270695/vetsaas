...
export type VacunaProductoOption = {
    id: string;
    nombre: string;
    sku: string | null;
    unidad: string | null;
...
type Props = {
    ...
    onSelect: (row: VacunaProductoOption | null) => void;
    disabled?: boolean;
    id?: string;
    'aria-invalid'?: boolean;
};

export function VacunaProductoPicker({
    value,
    labelResolved,
    onSelect,
    disabled = false,
    ...
    const { t } = useTranslation('vacunaciones');
    const [open, setOpen] = React.useState(false);
    const [search, setSearch] = React.useState('');
    const [loading, setLoading] = React.useState(false);
    const [options, setOptions] = React.useState<VacunaProductoOption[]>([]);

    const fetchUrl = React.useCallback((q: string) => {
        return clinica.vacunaciones.productosVacuna.url({
            query: { q: q.trim() },
        });
    }, []);
    ...
    React.useEffect(() => {
        ...
        const handle = window.setTimeout(() => {
            ...
            void fetch(fetchUrl(q), {
                ...
                .then(async (res) => {
                    const body = (await res.json()) as { data?: VacunaProductoOption[] };
