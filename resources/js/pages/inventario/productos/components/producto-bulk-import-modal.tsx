...
export function ProductoBulkImportModal({ open, onOpenChange }: Props) {
    const { t } = useTranslation('productos-inventario');
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await fetch('/inventario/productos/importar', {
                method: 'POST',
                ...
                <div className="grid min-h-0 flex-1 gap-0 overflow-hidden md:grid-cols-2">
                    <div className="flex flex-col gap-4 border-b border-border/60 p-6 md:border-r md:border-b-0">
                        ...
                        <ul className="space-y-1.5 text-xs text-muted-foreground">
                            <li>• {t('import.sheet_productos')}</li>
                            <li>• {t('import.sheet_categorias')}</li>
                            <li>• {t('import.sheet_unidades')}</li>
                            <li>• {t('import.required_hint')}</li>
                        </ul>

                        <Button asChild variant="outline" className="mt-auto w-full cursor-pointer gap-2">
                            <a href="/inventario/productos/plantilla-importacion" download>
