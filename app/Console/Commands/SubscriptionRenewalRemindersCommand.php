...
class SubscriptionRenewalRemindersCommand extends Command
{
    protected $signature = 'vetsaas:subscription-renewal-reminders
                            {--report : Muestra por qué se omitió cada suscripción de pago}
    ...
    public function handle(
        ...
        if (! $dryRun && ! $messenger->isReady()) {
            ...
            $this->line('Usa --dry-run --report para ver el diagnóstico sin WhatsApp.');
        ...
        } elseif ($result['scanned'] > 0 && $result['sent'] === 0 && ! $showReport) {
            $this->newLine();
            $this->comment('Ninguna suscripción cumple hoy las condiciones de envío.');
            $this->comment('Ejecuta con --report o --dry-run --report para ver el detalle por clínica.');
        }
